...
class ProgressBar {
     ...
     */
    private create() {
        const { size, arrow, block, whitespace } = this.options;
        ...
        const progressAfterArrow = whitespace ? ' '.repeat(emptyProgress) : block!.repeat(emptyProgress)
        const progressString = block!.repeat(progress) + arrow! + progressAfterArrow;
