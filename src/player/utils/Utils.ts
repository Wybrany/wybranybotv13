...
import AppleMusicSearch from "apple-music-metadata";
import { Track, RawPlaylist as AppleRawPlayList } from "apple-music-metadata";
...
export class Utils {
    ...
    static async link(Search: string, SOptions: PlayOptions = DefaultPlayOptions, Queue: Queue) {
        ...
        if (AppleLink) {
            try {
                let AppleResult = await AppleMusicSearch(Search) as Track;
                if (AppleResult) {
    ...
    static async playlist(Search: Playlist | string, SOptions: PlaylistOptions & { data?: any } = DefaultPlaylistOptions, Queue: Queue): Promise<Playlist> {
        ...
        if (ApplePlaylistLink) {
            ...
            let AppleResultData = await AppleMusicSearch(Search).catch(() => null) as AppleRawPlayList;
            if (!AppleResultData)
                throw DMPErrors.INVALID_PLAYLIST;

            let AppleResult: RawPlaylist = {
                name: AppleResultData.title,
                author: AppleResultData.creator.name,
                url: Search,
                songs: [],
